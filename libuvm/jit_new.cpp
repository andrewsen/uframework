        ...
        switch (*code) {
            case OpCode::ADD: {
                ...
                switch (type)
                {
                    ...
                    case Type::I32:
                    {
                        //x86code.push_back();
                        //x86code.push_back(POP_REG(EAX));
                        uint reg = reg_stack.FreeRev();
                        x86code.push_back(0b11); // FIX FOR ALL REGS (NOT EAX)
                        x86code.push_back(MRM(0b11, reg, reg_stack.FreeRev()));
                        reg_stack.Use(reg);
                        //x86code.push_back(PUSH_REG(EAX));
                    }
                        break;
            ...
            case OpCode::AND: {
                ...
                switch (type)
                {
                    ...
                    case Type::I32:
                    {
                        ...
                        //x86code.push_back(POP_REG(EAX));
                        uint reg = reg_stack.FreeRev();
                        x86code.push_back(0x23); // FIX FOR ALL REGS (NOT EAX)
                        x86code.push_back(MRM(0b11, reg, reg_stack.FreeRev()));
                        reg_stack.Use(reg);
                        //x86code.push_back(PUSH_REG(EAX));
                    }
...
#endif
                for(int i = 0; i < next->argc; ++i)
                {
                    x86code.push_back(PUSH_REG(reg_stack.FreeRev()));
                }
                //x86code.push_back(0xE8); //CALL...
                x86code.push_back(0xB8); // MOV...
            ...
            case OpCode::DEC:
            {
                ...
                switch(type){
                    ...
                    case Type::I32:
                    {
                        //x86code.push_back(POP_REG(EAX));
                        x86code.push_back(0x48);
                        x86code.push_back(reg_stack.GetRegRev());
                        break;
                    }
                    case Type::I64:
                    case Type::UI64:
            ...
            case OpCode::DIV: {
                ...
                switch (type) {
                    ...
                    case Type::UI32:
                    {
                        ...
                        //uint reg1 = reg_stack.FreeRev();
                        //uint reg2 = reg_stack.FreeRev();
                        udiv(reg_stack, x86code);

                        /*bool edx_on_stack = false;
                        if((reg1 != EDX && reg2 != EDX) && reg_stack.IsUsed(EDX))
                        {
                            edx_on_stack = saveReg(EDX, reg_stack, x86code);
                        }
                        if(reg1 != EAX && reg2 == EAX)
                        {
                            xchg(reg1, reg2, x86code);
                        }
                        else if(reg1 != EAX && reg2 != EAX)
                        {
                            if(reg_stack.IsUsed(EAX))
                                xchg(EAX, reg1, x86code);
                            else
                            {
                                reg_stack.Use(EAX);
                                mov(EAX, reg1, x86code);
                            }
                        }
                        // -> eax, ecx, edx | 0b00111000
                        // xchg eax, ecx
                        // div [ecx, edx]   | 0b00111110
                        // xchg eax, ecx
            ...
            case OpCode::DUP: {
                ...
                switch (type) {
                    ...
                    case Type::ARRAY:
                    {
                        uint reg1 = reg_stack.GetRegRev();
                        uint reg2 = reg_stack.Use();
                        mov(reg2, reg1, x86code);
                        //x86code.push_back(POP_REG(EAX));
                        //x86code.push_back(PUSH_REG(EAX));
                        //x86code.push_back(PUSH_REG(EAX));
                    }
            ...
            case OpCode::EQ: {
                ...
                switch (type) {
                    ...
                    case Type::I32:
                    {
                        uint reg1 = reg_stack.FreeRev();
                        uint reg2 = reg_stack.FreeRev();
                        //uint reg_tmp = reg_stack.Use();
                        //x86code.push_back(POP_REG(EAX));
                        //x86code.push_back(0x31); //XOR NULL ECX
                        //x86code.push_back(MRM(0b11, reg_tmp, reg_tmp));
                        //x86code.push_back(0x39);            //CMP
                        //x86code.push_back(MRM(0b11, reg1, reg2)); //..
                        //x86code.push_back(0x0F);
                        //x86code.push_back(0x90+0b0100); //SETE
                        //x86code.push_back(NNN(0b11, 0, reg_stack.Low8(re)));
                        //x86code.push_back(PUSH_REG(ECX));
                    }
                        break;
                    case Type::DOUBLE:
...
{
    if(reg_stack.IsAllUsed())
    {
        x86code.push_back(PUSH_REG(reg));
        return true;
    }
    mov(reg_stack.Use(), reg, x86code);
    return false;
}

void mov(uint reg1, uint reg2, vector<byte> &x86code)
{
    x86code.push_back(0x8B);
    x86code.push_back(MRM(0b11, reg1, reg2));
}

void xchg(uint reg1, uint reg2, vector<byte> &x86code)
{
    x86code.push_back(0x87);
    x86code.push_back(MRM(0b11, reg1, reg2));
}
...
void udiv(JITRegStack &reg_stack, vector<byte> &x86code)
{
    ...
    //      mov eax, eYx
    //      div eax, eXx; use(eax)
    throw JITException("Udiv is not implemented yet", __FUNCTION__);
}

void div(JITRegStack &reg_stack, vector<byte> &x86code)
{
    throw JITException("Div is not implemented yet", __FUNCTION__);
}
